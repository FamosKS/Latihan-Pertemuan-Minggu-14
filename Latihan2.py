...
Original file is located at
    https://colab.research.google.com/drive/1nLBjWY2sz6zLSnSlU0nTCteq0s0lw5oZ
"""
...
import warnings
import cv2 # Added this line to import OpenCV
...
def praktikum_transfer_learning():
    ...
    for i in range(10):
        plt.subplot(2, 5, i + 1)
        plt.imshow(X_train[i].astype(np.float32) / 255.0) # Normalize for display
    ...
    for i in range(5):
        ...
        plt.imshow(augmented[0].numpy().astype(np.float32) / 255.0) # Normalize for display
        plt.title("Augmented")
        plt.axis('off')
    ...
    callbacks = [
        ...
        keras.callbacks.ModelCheckpoint(
            'best_transfer_model.keras', # Changed from .h5 to .keras
            monitor='val_accuracy',
            save_best_only=True
        )
    ]
    ...
    # Handle ZeroDivisionError for f1_score if precision and recall are both 0
